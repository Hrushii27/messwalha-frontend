...
export const getAllMesses = async (req, res, next) => {
    try {
        ...
        let query = db.collection('messes').where('isVisible', '==', true);
        if (cuisine) {
        ...
        const messes = await Promise.all(messSnapshot.docs.map(async (doc) => {
...
export const getMessById = async (req, res, next) => {
    try {
        if (!db)
            return next(new AppError('Database not configured', 500));
        const messDoc = await db.collection('messes').doc(req.params.id).get();
        if (!messDoc.exists) {
...
export const getMyMess = async (req, res, next) => {
    try {
        if (!db)
            return next(new AppError('Database not configured', 500));
