...
export const getStats = async (req, res, next) => {
    try {
        ...
        const [usersSnapshot, messesSnapshot, activeSubsSnapshot, paymentsSnapshot, platformStatsDoc, ownerSubsSnapshot] = await Promise.all([
...
export const getAllMessesAdmin = async (req, res, next) => {
    try {
        if (!db)
            return next(new AppError('Database not configured', 500));
...
export const verifyMess = async (req, res, next) => {
    try {
        if (!db)
            return next(new AppError('Database not configured', 500));
        const { id } = req.params;
        const { verified } = req.body;
        await db.collection('messes').doc(id).update({ verified });
