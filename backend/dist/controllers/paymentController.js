...
export const createOwnerOrder = async (req, res, next) => {
    try {
        if (!db)
            return next(new AppError('Database not configured', 500));
        const amount = 499;
    ...
    try {
        ...
        try {
            ...
            if (userDoc.exists && messDoc.exists) {
                ...
                emailService.sendSubscriptionNotice(user.email, mess.name, endDate).catch((err) => {
...
export const verifyOwnerPayment = async (req, res, next) => {
    try {
        if (!db)
            return next(new AppError('Database not configured', 500));
